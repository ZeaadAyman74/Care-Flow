...
class SendRequestScreen extends StatefulWidget {
  const SendRequestScreen({Key? key, required this.doctorName,required this.doctorId})
      : super(key: key);
      ...
      body: BlocListener<SendRequestCubit,SendRequestState>(
        listener: (context, state) {
          if(state is SendRequestError){
            ...
            AppFunctions.showToast(message: 'Request sent successfully', state: ToastStates.success);
            context.pop();
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  BlocBuilder<SendRequestCubit,SendRequestState>(
                    buildWhen: (previous, current) => current is SendRequestLoad ||current is SendRequestError || current is SendRequestSuccess,
                    builder: (context, state) {
                        ...
                        return CommonButtons(
                          ...
                          onTap: () async {
                            if(formKey.currentState!.validate()){
                              if(SendRequestCubit.get(context).pickedImage==null){
                                AppFunctions.showMySnackBar(context, 'Please provide an X-ray image');
                              }else {
                                formKey.currentState!.save();
                                await SendRequestCubit.get(context).uploadImage(prevDiseases: preDiseases, doctorId: widget.doctorId, notes: notes);
                              }
                            }
                          },
                        );
